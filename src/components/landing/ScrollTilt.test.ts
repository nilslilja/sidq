...
/*
 * The pose no longer carries a scale.
 *
 * It used to go 0.94 to 1 alongside the rotation — six percent against an
 * eighteen degree tilt, and the most expensive thing on the page: this wraps
 * the desktop mock, and a subtree whose size changes is re-rasterised on every
 * frame of the scroll, backdrop-filtered dock and drop-shadowed icons included.
 * The rotation is the effect; it composites, and the six percent is not missed.
 */
describe("tiltPose", () => {
  ...
  test("is flat and opaque once the card is centred", () => {
