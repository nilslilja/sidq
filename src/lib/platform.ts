...
export type Platform =
  "macos-arm" | "macos-intel" | "windows" | "linux" | "phone" | "unknown";
 ...
 */
/**
 * What both the prerender and the first client render assume.
 *
 * The static HTML is built with no `navigator` to ask, and React throws away a
 * prerendered tree whose first client render disagrees with it, so the two have
 * to agree before detection has happened. Sidq runs on one kind of machine, so
 * that is the assumption worth starting from.
 */
export const ASSUMED_PLATFORM: PlatformInfo = describe("macos-arm");
...
export function detectPlatform(): PlatformInfo {
  ...
  if (isTouchMac || /iPhone|iPad|iPod|Android/i.test(ua))
    return describe("phone");

  if (platformHint.includes("win") || /Windows|Win64|Win32/i.test(ua))
    return describe("windows");
  ...
  if (platformHint.includes("linux") || /Linux|X11/i.test(ua))
    return describe("linux");
  ...
  switch (platform) {
    ...
    case "phone":
      return {
        ...
        detail:
          "Sidq is a Mac app. Leave your address and I will send it over.",
        supported: false,
      };
    default:
      return {
