...
pub fn install_id(conn: &rusqlite::Connection) -> String {
    ...
    let id = match std::fs::read("/dev/urandom") {
        // read() on urandom returns as much as it feels like; take what we get
        // and fall back to the clock rather than looping on a special file.
        Ok(bytes) if bytes.len() >= 16 => {
            bytes[..16].iter().map(|b| format!("{b:02x}")).collect::<String>()
        }
        _ => format!("{:032x}", index_store::now_millis()),
    };
